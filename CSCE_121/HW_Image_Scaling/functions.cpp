...
double map_coordinates(unsigned int source_dimension, unsigned int target_dimension, unsigned int pixel_coordinate){
  ...
  double map_coord;
  map_coord = (static_cast<double>(source_dimension - 1) / static_cast<double>(target_dimension - 1)) * pixel_coordinate;
  return map_coord;
}
...
void scale_image(Pixel sourceImage[][MAX_HEIGHT], unsigned int sourceWidth, unsigned int sourceHeight,
  ...
  for (unsigned int row = 0; row < targetHeight; row++) {
    for (unsigned int col = 0; col < targetWidth; col++) {
      x = map_coordinates(sourceWidth, targetWidth, col);
      y = map_coordinates(sourceHeight, targetHeight, row);
      if (targetHeight > sourceHeight && targetWidth > sourceWidth) {
        targetImage[col][row] = bilinear_interpolation(sourceImage, targetWidth, targetHeight, x, y);
      }
      else if (targetHeight <= sourceHeight && targetWidth <= sourceWidth) {
        targetImage[col][row] = bilinear_interpolation(sourceImage, sourceWidth, sourceHeight, x, y);
      }
    }
  }
}
